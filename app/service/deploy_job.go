...
func (this *DeployJob) CreateScript() (string, error) {
	...
	for _, ip := range ipList {
		if this.env.BeforeShell != "" {
			...
			// 这个分支是专门为了发布missfresh的AS的java系统
			if strings.Contains(this.env.BeforeShell, "deploy_service.sh") {
				beforeShell := this.env.BeforeShell + " " + this.project.Domain + " " + this.task.EndVer
				f.WriteString("ssh -o StrictHostKeyChecking=no -p " + serverPort + " -i " + serverKey + " " + serverUser + "@" + ip + " '" + beforeShell + "'\n\n")
			} else {
