...
CREATE TABLE `t_project` (
  ...
  `create_time` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `send_mail` int(2) NOT NULL DEFAULT '0' COMMENT '是否发送邮件',
  `mail_tpl_id` int(11) NOT NULL DEFAULT '0' COMMENT '邮件模板ID',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `t_task` (
  ...
  `start_ver` varchar(40) NOT NULL DEFAULT '' COMMENT '起始版本',
  `end_ver` varchar(40) NOT NULL DEFAULT '' COMMENT '结束版本',
